...
find_method:
	...
	if (method)
	{
		if (method->handler(b, tree_in, tree_out))
			goto error;
	}
	else
	{
								...
								xmlNodePtr tree_out)
{
	...
	if (object_name)
	{
		...
		if (external_action_handle(json_handle_add_object))
			goto fault_out;
	}
	else
	{
