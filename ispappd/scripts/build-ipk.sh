...
show_help() {
    ...
    echo "  $0 --arch x86_64 --keep-running    # Keep container running for inspection"
    echo ""
...
print_status "Keep running: $KEEP_RUNNING"
...
EOF
    ...
    if [[ "$KEEP_RUNNING" == true ]]; then
        # Run with interactive mode and without --rm for inspection
        DOCKER_ARGS=(
            "-it"
            "--name" "ispappd-build-$(date +%s)"
            "-v" "$(pwd):/source:ro"
        ...
        print_status "Running in keep-running mode..."
        print_status "Container will remain active after build for dependency inspection"
        
        # Run docker and capture its exit status
        ...
        if [ $DOCKER_EXIT_CODE -eq 0 ]; then
            ...
            print_status "Use 'docker exec -it $CONTAINER_NAME /bin/bash' to connect"
            print_status "Use 'docker stop $CONTAINER_NAME' to stop when done"
        else
