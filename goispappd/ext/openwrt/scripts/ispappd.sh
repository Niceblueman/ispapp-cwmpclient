...
DOWNLOAD_DIR="/tmp/ispappd_download"
ISPAPPD_PROMPT="ispappd>"
set_fault_tmp_file="/tmp/.ispappd_set_fault_tmp"
apply_service_tmp_file="/tmp/.ispappd_apply_service"
set_command_tmp_file="/tmp/.ispappd_set_command_tmp"
FUNCTION_PATH="/usr/share/ispappd/functions"
NOTIF_PARAM_VALUES="/tmp/.ispappd_notif_param_value"
ispappd_config_changed=""
uci_change_packages=""
uci_change_services=""
g_fault_code=""
...
# TODO: check firmaware size with falsh to be improved  
	...
	if [ "$action" = "upload" ]; then 
		...
		local up_url="$__arg1"
		SERIAL_NUMBER=`$UCI_GET ispappd.@device[0].serial_number`
	...
	if [ "$action" = "apply_download" ]; then
		if [ "$__arg1" = "3 Vendor Configuration File" ]; then 
			...
			local gr_backup=`grep "^/etc/ispappd/\.backup\.xml" /etc/sysupgrade.conf`
			[ -z $gr_backup ] && echo "/etc/ispappd/.backup.xml" >> /etc/sysupgrade.conf
			dwfile=`ls $DOWNLOAD_DIR`
			if [ "$dwfile" != "" ]; then
				dwfile="$DOWNLOAD_DIR/$dwfile"
		...
		if [ ! -f "$set_fault_tmp_file" ]; then
			...
			if [ -n "$rev" ]; then
				...
				if [ "$action" = "apply_value" ]; then
					...
					$UCI_SET ispappd.@acs[0].parameter_key="$__arg1"
					common_json_output_status "1"
				fi
